...
def train_on_batch(batch, batch_size=batch_size):
    ...
    #     s_ = torch.tensor([b[3] for b in batch], dtype=torch.float32).to(device)
    #     q_ = net(s_)
    #     max_q = torch.max(q_, keepdim=False, dim=1).values
    #
    #     targets = torch.clone(q)
    #     targets_1d = targets.flatten()
    #
    #     actions = torch.tensor([b[1] for b in batch], dtype=torch.long).to(device)
    #     actions = actions + torch.arange(batch_size, dtype=torch.long).to(device) * 3
    #     rewards = torch.tensor([b[2] for b in batch], dtype=torch.long).to(device)
    #     done = torch.tensor([b[4] for b in batch]).to(device)
    #
    #     targets_1d[actions] = rewards + discount_factor * max_q * (1 - done.float())
    #     y = targets_1d.reshape((batch_size, 3))
